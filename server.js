        ...
        Object.values(game.players).forEach(player => {
            ...
            if (player.lives > 0) {
                if (game.settings.answerType === 'multiple') {
                    if(guess.title === song.title) player.lastPointsBreakdown.title = MAX_POINTS_TITLE;
                    if(guess.artist === song.artist) player.lastPointsBreakdown.artist = MAX_POINTS_ARTIST;
                    if(parseInt(guess.year) === song.year) player.lastPointsBreakdown.year = MAX_POINTS_YEAR;
                } else {
                    ...
                    if (artistDist <= 3) player.lastPointsBreakdown.artist = Math.round(MAX_POINTS_ARTIST * (1 - (artistDist / 4)));
                    if (titleDist <= 4) player.lastPointsBreakdown.title = Math.round(MAX_POINTS_TITLE * (1 - (titleDist / 5)));
                    if (!isNaN(yearDiff) && yearDiff <= 10) player.lastPointsBreakdown.year = Math.round(MAX_POINTS_YEAR * (1 - (yearDiff / 11)));
                }
