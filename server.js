...
app.get('/api/devices', async (req, res) => {
    ...
    if (!token) return res.status(401).json({ message: "Nicht autorisiert" });
    try {
        const d = await axios.get('http://googleusercontent.com/spotify.com/8', { headers: { 'Authorization': `Bearer ${token}` } });
        res.json(d.data);
    } catch (e) {
...
wss.on('connection', ws => {
    ...
    ws.on('message', message => {
        try {
            ...
            console.error("Fehler bei WebSocket-Nachricht:", e);
        }
    });
    ...
    switch (type) {
        ...
        case 'join-game':
            ...
            if (gameToJoin && gameToJoin.gameState === 'LOBBY') {
                ws.pin = payload.pin;
                gameToJoin.players[playerId] = { ws, nickname: payload.nickname, score: 0 };
...
async function startGame(pin) {
    ...
    const deviceId = game.settings.deviceId;
    ...
    try {
        ...
        broadcastToLobby(pin, { type: 'error', payload: { message: 'Ausgewähltes Gerät nicht aktiv.' } });
        return;
    }
...
async function getPlaylistTracks(playlistId, token) {
    try {
        ...
        return response.data.items.map(item => item.track).filter(track =>
            track && track.id && track.name && track.artists[0] && track.artists[0].name && track.album && track.album.release_date
        ).map(track => ({
            spotifyId: track.id,
            title: track.name,
        ...
        console.error("Fehler bei Playlist-Tracks:", error.message);
        return null;
    }
}

server.listen(process.env.PORT || 8080, () => {
    console.log(`✅ Fakester-Server läuft auf Port ${process.env.PORT || 8080}`);
});
